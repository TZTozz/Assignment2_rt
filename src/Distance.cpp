...
class Distance : public rclcpp::Node
{
    public:
        Distance() : Node("distance_node")
        {
            ...
            threshold_server_ = this->create_service<turtlesim_custom_msgs::srv::SetThreshold>("set_threshold", std::bind(&Distance::set_threshold, this, _1, _2) );
    ...
    private:
        ...
        void timer_callback()
        {
            ...
            if (index != -1) {
                ...
                RCLCPP_INFO(this->get_logger(), "Obstacle at %.2f m, Angle: %.2f deg", min_range, msg_distance.angle);
            } else {
            ...
            if (min_range < threshold_ && index != -1)
            {
                if (!alreadyStopped) {
                    ...
                    RCLCPP_WARN(this->get_logger(), "STOP! Obstacle too close: %.2f m", min_range);
                    alreadyStopped = true;
                }
            }
            else
            {
                if (alreadyStopped)
                {
                    msg_stop.data = false;
                    publisherStop_->publish(msg_stop);
                    RCLCPP_INFO(this->get_logger(), "No more near to an obstacle");
                    alreadyStopped = false;
                }
            }
        }

        void set_threshold(const std::shared_ptr<turtlesim_custom_msgs::srv::SetThreshold::Request> request,
                            std::shared_ptr<turtlesim_custom_msgs::srv::SetThreshold::Response> response)
        {
    ...
    rclcpp::Publisher<turtlesim_custom_msgs::msg::DistanceAngle>::SharedPtr publisherDistance_;
    rclcpp::Publisher<std_msgs::msg::Bool>::SharedPtr publisherStop_;
    rclcpp::Service<turtlesim_custom_msgs::srv::SetThreshold>::SharedPtr threshold_server_;
